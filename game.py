...
import os
from config import *
from utils import snake_size, time_elapsed, format_cell, color, food_loc_generate, change_velocity, log_game, high_score, score_update, check_lose, snake_move, level_name, power_loc_generate, power_func, setup_color, key_display
import queue as q 
...
def main(stdscr:any)->None:
    ...
    while(game_loop):
        ...
        if (highest_score[0]<score[0]):
            if highest_score[0]:
                stdscr.addstr(14,0,f"Congratulations! You just broke the highest record of {score[0]}")
            else:
